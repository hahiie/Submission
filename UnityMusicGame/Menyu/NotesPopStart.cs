...
public class NotesPopStart : MonoBehaviour
{
    ...
    void Pop()
    {
        ...
        //リソースから読み込む
        string datastr = "";
        jsonFile = Resources.Load("song" + SongControalStart.SongNumber + "_difficulty" + SongControalStart.SongDifficulty) as TextAsset;              //指定したファイルをTextAssetとして読み込み(ファイル名の.csvは不要なことに注意)　最初の行（タイトル部分）も読み込まれるのでそこは使用しない
        ...
        for (i = 0; i < noteslist.notes.Length; i++)
        {
            if (noteslist.notes[i].type == 1)//ノーマルノーツ
            {
                ...
                NotesMoveStart notesMove = a.GetComponent<NotesMoveStart>(); NomalNotesStart nomalNotes = a.GetComponent<NomalNotesStart>();//スクリプト参照
            ...
            if (noteslist.notes[i].type == 2)//長押しOK
            {
                ...
                NotesMoveStart notesMove = a.GetComponent<NotesMoveStart>(); Type2NotesStart type2Notes = a.GetComponent<Type2NotesStart>();//スクリプト参照
            ...
            if (noteslist.notes[i].type == 3)//ロングノーツ
            {
                ...
                LongNotesStart type3Notes = a.GetComponent<LongNotesStart>();//スクリプト参照
